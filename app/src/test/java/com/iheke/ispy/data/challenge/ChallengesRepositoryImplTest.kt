...
class ChallengesRepositoryImplTest {
    ...
    private lateinit var repositoryImpl: ChallengesRepositoryImpl
    private lateinit var mockRemoteDataSource: ChallengesRemoteDataSource

    @Before
    fun setup() {
        mockRemoteDataSource = mockk()
        repositoryImpl = ChallengesRepositoryImpl(mockRemoteDataSource)
    }
    ...
    fun `getChallenges returns list of ChallengesApiModel`() = runBlocking {
        ...
        // Act
        val actualChallenges = repositoryImpl.getChallenges().toList()
